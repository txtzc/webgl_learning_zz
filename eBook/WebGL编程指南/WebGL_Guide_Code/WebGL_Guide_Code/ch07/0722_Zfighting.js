...
function main() {
  ...
  // Enable the polygon offset function
	gl.enable(gl.POLYGON_OFFSET_FILL);// ZC: 多边形偏移   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--
  // Draw the triangles
  gl.drawArrays(gl.TRIANGLES, 0, n/2);   // The green triangle
//	gl.polygonOffset(1.0, 1.0);          // Set the polygon offset// ZC: 绘画前指定用来计算偏移量的参数 (只注释这一行，70行不注释，就会有"深度冲突"的现象差生)  <--   <--   <--   <--   <--   <--   <--   <--   <--   <--   <--
...
*/
  var verticesColors = new Float32Array([
    // Vertex coordinates and color
     0.0,  2.5,  -5.0,  1.0,  0.0,  0.0, // The green triangle
    -2.5, -2.5,  -5.0,  1.0,  0.0,  0.0,
     2.5, -2.5,  -5.0,  1.0,  0.0,  0.0, 

     0.0,  3.0,  -5.0,  0.0,  0.0,  1.0, // The yellow triagle
